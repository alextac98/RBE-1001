...
Drivetrain::Drivetrain():
  ...
  rightEncoder(ENCODERRIGHT1, ENCODERRIGHT2)
  {
    ...
    leftPID -> SetMode(MANUAL); //PID disabled by default
    ...
    rightPID -> SetMode(MANUAL);  //PID disabled by default
...
void Drivetrain::setDistance(int left,int right) { //inch
  ...
  //Enable PID
  //enableDISTPID();
  leftInputDIST = leftEncoder.read() * DIST_PER_TICK;
  rightInputDIST = rightEncoder.read() * DIST_PER_TICK;  
  rightDistancePID->Compute(); //compute PID
  leftDistancePID->Compute(); //compute PID
  setPower(leftOutputDIST,rightOutputDIST);
  Serial.println("Hi mom");
